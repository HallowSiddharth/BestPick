...
def transform_to_sentiment_score(
    log_likelihood_strongly_negative,
    log_likelihood_negative,
    log_likelihood_neutral,
    log_likelihood_positive,
    log_likelihood_strongly_positive,
):
    # Transform log likelihood to score using the sigmoid function
    score_negative = 1 / (1 + math.exp(-log_likelihood_negative))
    score_neutral = 1 / (1 + math.exp(-log_likelihood_neutral))
    score_positive = 1 / (1 + math.exp(-log_likelihood_positive))
    score_strongly_positive = 1 / (1 + math.exp(-log_likelihood_strongly_positive))
    score_strongly_negative = 1 / (1 + math.exp(-log_likelihood_strongly_negative))

    return (
        score_strongly_negative,
        score_negative,
        score_neutral,
        score_positive,
        score_strongly_positive,
    )
...
# training data
df = pd.read_csv("smash/dataset.csv")
X_train = list(df["Text"])
y_train = list(df["Score"])

# test data
X_test = ["This is a fantastic product", "I don't like this product at all"]
...
filename = "multinomial_nb_model2.pkl"
